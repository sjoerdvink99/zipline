...
export function computeGraphHash(nodes: D3Node[], edges: D3Edge[]): string {
  ...
  const edgeIds = edges
    ...
    .sort()
    .join(",");
  return `${nodes.length}:${edges.length}:${nodeIds.slice(0, 100)}:${edgeIds.slice(0, 100)}`;
}
...
function detectStructuralCommunities(
  ...
  // Use node_type as community when nodes carry meaningful type diversity.
  // This is always a better signal than label propagation: it reflects
  // domain semantics (e.g. CORA categories, BRON entity types) rather than
  // noisy topological clusters.
...
export function runStableForceLayout({
...
}: ForceLayoutOptions): d3.Simulation<D3Node, D3Edge> {
  ...
  const sim = d3
    ...
    .force(
      "center",
      d3.forceCenter<D3Node>(cx, cy).strength(K <= 1 ? 0.04 : 0.01),
    )
    .force(
      "collide",
      d3.forceCollide<D3Node>(collideRadius).strength(0.7).iterations(2),
    )
...
export function runForceAtlas2Layout({
  ...
  // Size-tiered parameters.
  // scalingRatio kept low so that node positions stay within a navigable
  // coordinate range (~10× the initial canvas size at most).
  // slowDown=3 prevents oscillation in dense graphs and improves convergence.
  ...
  // Community-aware golden-spiral initialization (same pattern as force layout)
  const communityMap = detectStructuralCommunities(nodes, edges);
  const communityGroups = new Map<string, D3Node[]>();
  for (const node of nodes) {
    const c = communityMap.get(node.id) ?? "cluster_0";
  ...
  // Write initial pinned positions to graphology
  for (const node of nodes) {
  ...
  const runChunk = () => {
    ...
    // Read positions back to D3Nodes
    for (const node of nodes) {
      if (!graph.hasNode(node.id)) continue;
      if (node.fx != null && node.fy != null) {
        // Re-pin fixed nodes in the graphology graph
        graph.setNodeAttribute(node.id, "x", node.fx);
        graph.setNodeAttribute(node.id, "y", node.fy);
        node.x = node.fx;
        node.y = node.fy;
