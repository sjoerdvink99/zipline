...
interface FilterItem {
  ...
  predicate: Record<string, string | number | boolean | undefined>;
  description: string;
  nodeTypes?: string[];
}
...
export const usePredicateComposerStore = create<PredicateComposerState>(
  (set, get) => ({
    ...initialState,
    ...
    removePredicate: (predicateId) => {
      const { queryCanvas } = get();
      const updatedPredicates = queryCanvas.predicates.filter(
        (p) => p.id !== predicateId,
      );

      const updatedConstraints = queryCanvas.neighbourhoodConstraints
        .map((constraint) => ({
          ...constraint,
          targetPredicateIds: constraint.targetPredicateIds.filter(
            (id) => id !== predicateId,
          ),
        }))
    ...
    updateNeighborhoodConstraint: (constraintId, updates) => {
      const { queryCanvas } = get();
      const updatedConstraints = queryCanvas.neighbourhoodConstraints.map(
        (c) => (c.id === constraintId ? { ...c, ...updates } : c),
      );
    ...
    removeNeighborhoodConstraint: (constraintId) => {
      const { queryCanvas } = get();
      const updatedConstraints = queryCanvas.neighbourhoodConstraints.filter(
        (c) => c.id !== constraintId,
      );
    ...
    removeProjectionVariable: (variableName) => {
      const { projection } = get();
      const updatedVariables = projection.variables.filter(
    ...
    evaluateExpression: async () => {
      ...
      try {
        const request: CompositePredicateRequest = {
          ...
          logical_structure: {
            type: "compound",
            operator: queryCanvas.logicalStructure.operator || "and",
            predicates: queryCanvas.logicalStructure.predicates || [],
          },
          neighborhood_constraints: queryCanvas.neighbourhoodConstraints.map(
            serializeNeighborhoodBlock,
          ) as unknown as NeighborhoodBlock[],
        ...
        const response: CompositePredicateResponse =
          await evaluateCompositePredicate(request);
