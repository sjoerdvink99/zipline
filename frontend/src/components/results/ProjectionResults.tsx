    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-semibold text-blue-800">
            {resultMode === 'primary_only' ? 'Primary Results Only' : 'Primary + Projected Results'}
          </h4>
        </div>
      ...
      <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-slate-700">
            {resultMode === 'primary_only' ? (
              <span>
                <span className="font-medium">Primary only:</span> Results show entities that satisfy the predicate directly.
                Neighborhood constraints act as <span className="font-mono bg-slate-200 px-1 rounded">witnesses</span> (validation only).
              </span>
            ) : (
              <span>
                <span className="font-medium">Primary + projected:</span> Results include both entities that satisfy the predicate
                and their <span className="font-mono bg-slate-200 px-1 rounded">projected neighbors</span> that participate in the pattern.
              </span>
            )}
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {projections.map((projection, index) => (
              <button
                ...
                onClick={() => handleNodeClick([projection.primary_node], 'primary')}
      ...
      {resultMode === 'primary_and_projected' && (
        ...
        {projections.map((projection, index) => {
          ...
          return (
            <div key={`${projection.primary_node}-${index}`} className="border border-gray-200 rounded">
              <div
                ...
                <div className="border-t border-gray-100 p-2 bg-gray-50">
                  ...
                  <div className="mt-2 pt-2 border-t border-gray-200">
                    <button
                      ...
                      onClick={() => {
                        const allProjectedNodes = Object.values(projection.projected_variables).flat();
                        handleNodeClick([projection.primary_node, ...allProjectedNodes], 'primary');
                      }}
                    >
      ...
      <div className="p-3 bg-slate-100 border border-slate-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-slate-700">
            <span className="font-medium">Interaction:</span> Click on entity names to highlight them in the graph.
            {resultMode === 'primary_and_projected' && (
              <span> Expand relations to see projected variables and their connections.</span>
            )}
          </div>
        </div>
      </div>
...
export function createStructuredProjectionResult(
  ...
  projectedRelations: Array<{ variable: string; nodes: string[]; relation_type: string }>,
  resultMode: 'primary_only' | 'primary_and_projected'
