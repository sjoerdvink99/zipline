...
export class GraphRenderer {
  ...
  private constellationMode = false; // true when too many path nodes to draw edges readably
  ...
  setPath(
    ...
    this.paths = paths;
    // Constellation mode: too many nodes to draw individual path edges readably
    ...
    if (this.constellationMode) {
      ...
      for (const path of paths) {
        for (let i = 0; i < path.length - 1; i++) {
          const edge = { source: path[i], target: path[i + 1] };
          if (
            !computed.some(
  ...
  private render(): void {
    ...
    if (this.nodeContainer) {
      ...
      const dimmedBatch = (hasFocusNodes || hasPathNodes) ? new PIXI.Graphics() : null;
      if (dimmedBatch) dimmedBatch.alpha = hasFocusNodes ? 0.12 : 0.18;
      ...
      for (const [nodeType, typeNodes] of this.nodesByTypeCache) {
        ...
        for (const node of typeNodes) {
          ...
          const targetBatch = dimThisNode ? dimmedBatch! : batchGraphics;
      ...
      if (dimmedBatch) this.nodeContainer.addChild(dimmedBatch);
    ...
    if (this.labelContainer) {
      ...
      if (showLabels) {
        for (const node of this.nodes) {
          ...
          text.position.set(nx, ny + radius + 3);
  ...
  exportHighRes(scale: number = 2, selectedOnly: boolean = false, dimBackground: boolean = false): HTMLCanvasElement | null {
    ...
    if (selectedOnly && this.selectedNodes.size > 0) {
      ...
      this.computeDegreeMap();
      this.updateAttributeRange();
      // Clear rings — all visible nodes are selected, rings would be noise
