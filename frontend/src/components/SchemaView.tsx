...
export const SchemaView = ({ onSelectionChange }: SchemaViewProps) => {
  ...
  useEffect(() => {
    ...
    const zoomBehavior = zoom().scaleExtent([0.1, 10]).on("zoom", (event) => {
    ...
    const simulation = forceSimulation(schemaData.schemaNodes as any)
      .force("link", forceLink(schemaData.schemaEdges as any).id((d: any) => d.id).distance(linkDistance).strength(0.4))
      .force("charge", forceManyBody().strength(chargeStrength).distanceMax(400))
      .force("center", forceCenter(centerX, centerY).strength(0.05))
      .force("collision", forceCollide().radius(collisionRadius).strength(0.9).iterations(3))
