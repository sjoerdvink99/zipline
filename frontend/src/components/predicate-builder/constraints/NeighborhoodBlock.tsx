...
import type { NeighborhoodConstraint, FilterItem, ConstraintItem } from '../../../types/predicate';
import { AutocompleteInput } from '../../ui/AutocompleteInput';
import { useGraphSchema, getAttributeSuggestions, getValueSuggestions } from '../../../hooks/useGraphSchema';
...
function ConstraintValidationIndicator({
  ...
  constraint: { attribute: string; operator: string; value: string };
  targetPredicateIds: string[];
}) {
  const { validation, loading, willHaveResults, matchingNeighbors, totalNeighbors } = useConstraintValidation({
    targetPredicateIds,
  ...
  const percentage = totalNeighbors > 0 ? Math.round((matchingNeighbors / totalNeighbors) * 100) : 0;
...
export function NeighborhoodBlock({
  ...
  const [editing, setEditing] = useState<NeighborhoodConstraint>(() => {
    ...
    if (migrated.level === undefined) {
      migrated.level = 1; // Default to level 1 (y variable)
    }
  ...
  const getTargetInfo = () => {
    if (constraint.targetType === 'all_predicates') {
      return { type: 'all_predicates', items: predicates, count: predicates.length };
    } else if (constraint.targetType === 'constraints' || constraint.parentConstraintId) {
      return { type: 'constraints', items: [], count: 1, parentId: constraint.parentConstraintId };
    }
    const targetPredicates = predicates.filter(p => constraint.targetPredicateIds.includes(p.id));
    return { type: 'predicates', items: targetPredicates, count: targetPredicates.length };
  };
  ...
  const convertFilterItemToConstraint = useCallback((filterItem: FilterItem): ConstraintItem => {
    ...
    return {
      ...
      type: filterItem.type === 'topology' ? 'topology' : 'attribute',
      attribute,
      operator,
      value,
      node_type: nodeType,
  ...
  const handlePillDrop = useCallback((draggedItem: any, targetIndex?: number) => {
    if (draggedItem.type === 'filter-item') {
      const newConstraint = convertFilterItemToConstraint(draggedItem.filterItem);
  ...
  const neighborSuggestions = useNeighborSuggestions({
    ...
    enabled: firstConstraint?.type === 'attribute' && !!firstConstraint.attribute
  });

  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    if (isEditing) return; // Don't show context menu when already editing
  ...
  const handleContextMenuSave = useCallback((updatedConstraint: NeighborhoodConstraint) => {
  ...
  if (isEditing) {
    return (
      <div ref={blockRef} className="border-2 border-violet-300 rounded-lg p-4 bg-violet-50/50">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-violet-800">Edit Neighborhood Constraint</h3>
            <div className="flex gap-2">
              <button onClick={handleSave} className="px-3 py-1 text-xs font-medium bg-violet-600 text-white rounded hover:bg-violet-700 transition-colors">Save</button>
              <button onClick={handleCancel} className="px-3 py-1 text-xs text-gray-600 hover:text-gray-800 transition-colors">Cancel</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditing(prev => ({ ...prev, quantifier: e.target.value as any }))}
            ...
            {['EXACTLY', 'AT_LEAST', 'AT_MOST'].includes(editing.quantifier) && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditing(prev => ({ ...prev, count: parseInt(e.target.value) || 1 }))}
                  className="w-full text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                />
              </div>
            )}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditing(prev => ({ ...prev, relation: e.target.value as any }))}
                className="w-full text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
              >
            ...
            {editing.relation === 'k_hop' && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditing(prev => ({ ...prev, kParameter: parseInt(e.target.value) || 2 }))}
                  className="w-full text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                />
              </div>
            )}
          ...
          <div>
            ...
            <div
              ...
              onDragOver={(e) => {
                e.preventDefault();
                e.stopPropagation(); // Prevent parent drop zones from handling this
            ...
            >
              <div className="text-xs text-gray-500 mb-2">Drop attribute/topology pills here or configure manually:</div>
                ...
                <div key={c.id} className="space-y-2">
                  <div className="grid grid-cols-5 gap-2">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        newConstraints[index] = { ...c, type: e.target.value as 'attribute' | 'topology' | 'nested_pill' };
                        setEditing(prev => ({ ...prev, constraints: newConstraints }));
                      }}
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const newConstraints = [...editing.constraints];
                        newConstraints[index] = { ...c, attribute: e.target.value };
                        setEditing(prev => ({ ...prev, constraints: newConstraints }));
                      }}
                      className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                    ...
                    >
                      <option value="">
                        {schemaLoading ? 'Loading attributes...' : 'Select attribute...'}
                      </option>
                      {!schemaLoading &&
                        getAttributeSuggestions(schema, c.type === 'topology' ? 'topology' : 'attribute').map((attr) => (
                          <option key={attr} value={attr}>
                            {attr}
                          </option>
                        ))
                      }
                      {!schemaLoading && schema && getAttributeSuggestions(schema, c.type === 'topology' ? 'topology' : 'attribute').length === 0 && (
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const newConstraints = [...editing.constraints];
                        newConstraints[index] = { ...c, operator: e.target.value };
                        setEditing(prev => ({ ...prev, constraints: newConstraints }));
                      }}
                      className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                    >
                    ...
                    <div className="relative">
                      <AutocompleteInput
                        ...
                        suggestions={
                          c.type === 'attribute' && c.attribute === firstConstraint?.attribute && neighborSuggestions.values.length > 0
                            ? neighborSuggestions.values
                            : getValueSuggestions(schema, c.attribute, c.type === 'topology' ? 'topology' : 'attribute')
                        }
                        placeholder="value"
                        className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                      />
                    ...
                    <button
                      onClick={() => {
                        const newConstraints = editing.constraints.filter((_, i) => i !== index);
                        setEditing(prev => ({ ...prev, constraints: newConstraints }));
                      }}
                  ...
                  {index < editing.constraints.length - 1 && (
                    <div className="flex items-center gap-2">
                      <div className="text-xs text-gray-500">AND/OR</div>
                      <select
                        value={c.combineOp || 'and'}
                        onChange={(e) => {
                          const newConstraints = [...editing.constraints];
                          newConstraints[index] = { ...c, combineOp: e.target.value as 'and' | 'or' };
                          setEditing(prev => ({ ...prev, constraints: newConstraints }));
                        }}
                        className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                      >
              ...
              <div className="space-y-2">
                <label className="flex items-center text-xs cursor-pointer">
                  <input
                    ...
                    onChange={() => setEditing(prev => ({ ...prev, resultMode: 'primary_only', projectionVariable: undefined }))}
                ...
                <label className="flex items-center text-xs cursor-pointer">
                  <input
                    ...
                    onChange={() => setEditing(prev => ({ ...prev, resultMode: 'primary_and_projected', projectionVariable: prev.projectionVariable || 'neighbor' }))}
                    className="mr-2 text-violet-600 focus:ring-violet-500"
                  />
            ...
            {editing.resultMode === 'primary_and_projected' && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditing(prev => ({ ...prev, projectionVariable: e.target.value || 'neighbor' }))}
          ...
          <div className="space-y-3 pt-3 border-t border-violet-200">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-medium text-gray-700">
                Nested Constraints <span className="text-gray-500">({getVariableForLevel(editing.level + 1)} level)</span>
              </label>
              <button
                onClick={() => {
                  ...
                  setEditing(prev => ({
                    ...prev,
                    nestedConstraints: [...(prev.nestedConstraints || []), newNestedConstraint]
                  }));
                }}
                className="px-2 py-1 text-xs bg-violet-100 text-violet-700 border border-violet-200 rounded hover:bg-violet-200 transition-colors"
              >
            ...
            <div className="text-xs text-gray-500 mb-2">
              Add constraints on the neighbors of the current constraint's matching nodes.
              For example: find nodes with neighbors that have property A, and those neighbors also have neighbors with property B.
            </div>

            {editing.nestedConstraints && editing.nestedConstraints.length > 0 && (
                  ...
                  <div key={nestedConstraint.id} className="relative">
                    <div className="flex items-start justify-between mb-2">
                      <div className="text-xs font-medium text-violet-800">
                        Level {nestedConstraint.level} ({getVariableForLevel(nestedConstraint.level)})
                      </div>
                      <button
                        onClick={() => {
                          const newNestedConstraints = editing.nestedConstraints?.filter((_, i) => i !== index) || [];
                          setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                        }}
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      <select
                        ...
                        onChange={(e) => {
                          const newNestedConstraints = [...(editing.nestedConstraints || [])];
                          newNestedConstraints[index] = { ...nestedConstraint, quantifier: e.target.value as any };
                          setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                        }}
                        className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                      >
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newNestedConstraints = [...(editing.nestedConstraints || [])];
                          newNestedConstraints[index] = { ...nestedConstraint, relation: e.target.value as any };
                          setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                        }}
                        className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                      >
                        ...
                        <option value="connected_components">connected</option>
                      </select>
                    </div>

                    {['EXACTLY', 'AT_LEAST', 'AT_MOST'].includes(nestedConstraint.quantifier) && (
                      <div className="mb-2">
                        <input
                          type="number"
                          min="1"
                          ...
                          onChange={(e) => {
                            const newNestedConstraints = [...(editing.nestedConstraints || [])];
                            newNestedConstraints[index] = { ...nestedConstraint, count: parseInt(e.target.value) || 1 };
                            setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                          }}
                    ...
                    <div className="space-y-2">
                      {nestedConstraint.constraints.map((c, constraintIndex) => (
                        <div key={c.id} className="grid grid-cols-4 gap-2">
                          <select
                            value={c.attribute}
                            onChange={(e) => {
                              const newNestedConstraints = [...(editing.nestedConstraints || [])];
                              const newConstraints = [...newNestedConstraints[index].constraints];
                              newConstraints[constraintIndex] = { ...c, attribute: e.target.value };
                              newNestedConstraints[index] = { ...nestedConstraint, constraints: newConstraints };
                              setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                            }}
                            className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                          >
                            <option value="">Select attribute...</option>
                            {getAttributeSuggestions(schema, 'attribute').map((attr) => (
                              <option key={attr} value={attr}>
                                {attr}
                              </option>
                            ))}
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              const newNestedConstraints = [...(editing.nestedConstraints || [])];
                              const newConstraints = [...newNestedConstraints[index].constraints];
                              newConstraints[constraintIndex] = { ...c, operator: e.target.value };
                              newNestedConstraints[index] = { ...nestedConstraint, constraints: newConstraints };
                              setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                            }}
                            className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const newNestedConstraints = [...(editing.nestedConstraints || [])];
                              const newConstraints = [...newNestedConstraints[index].constraints];
                              newConstraints[constraintIndex] = { ...c, value: e.target.value };
                              newNestedConstraints[index] = { ...nestedConstraint, constraints: newConstraints };
                              setEditing(prev => ({ ...prev, nestedConstraints: newNestedConstraints }));
                            }}
                            className="text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
                          ...
                          <button
                            onClick={() => {
                              const newNestedConstraints = [...(editing.nestedConstraints || [])];
                              const newConstraints = newNestedConstraints[index].constraints.filter((_, i) => i !== constraintIndex);
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-md border text-xs font-medium ${quantifierColors[constraint.quantifier]}`}>
          ...
          <div className="text-xs text-violet-700 font-mono">{folExpression}</div>
        </div>

        <button onClick={onRemove} className="p-1 text-red-500 hover:text-red-700 hover:bg-red-100 rounded transition-colors">
          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
          ...
          <div className="flex flex-wrap gap-1.5">
            {targetInfo.type === 'all_predicates' ? (
              ...
              <div className="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs bg-purple-100 text-purple-800 border-purple-200">
                ...
                <span>Previous Constraint{targetInfo.parentId ? ` (${targetInfo.parentId})` : ''}</span>
              </div>
            ) : (
              targetInfo.items.map(pred => {
                ...
                return (
                  <div key={pred.id} className={`inline-flex items-center gap-1 px-2 py-1 rounded border text-xs ${styles}`}>
                    <span className="truncate max-w-[100px]">{pred.description}</span>
                  </div>
                );
              })
            )}
          ...
          <div
            ...
            >
              {(constraint.constraints || []).length === 0 ? (
                <div className="text-xs text-gray-400 italic">Drop attribute/topology pills here</div>
              ) : (
                (constraint.constraints || []).map((constraintItem, index) => (
                  <NestedConstraintPill
                    ...
                    onOperationChange={(op) => {
                      const newConstraints = [...(constraint.constraints || [])];
                      newConstraints[index] = { ...constraintItem, combineOp: op };
                      onUpdate({ ...constraint, constraints: newConstraints });
                    }}
                    onRemove={() => {
                      const newConstraints = (constraint.constraints || []).filter((_, i) => i !== index);
                      onUpdate({ ...constraint, constraints: newConstraints });
                    }}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-1">
              ...
              <span className={constraint.resultMode === 'primary_and_projected' ? 'text-amber-700 font-medium' : 'text-gray-600'}>
                {constraint.resultMode === 'primary_only' ? 'Primary only' : `Primary + ${constraint.projectionVariable || 'neighbors'}`}
              </span>
            </div>
          </div>
