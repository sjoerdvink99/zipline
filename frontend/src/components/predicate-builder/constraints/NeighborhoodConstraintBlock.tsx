...
export function NeighborhoodConstraintBlock({
  ...
  const getTargetItems = useCallback(() => {
    ...
    return filterItems.filter((item) =>
      block.targetPredicateIds.includes(item.id),
    );
            ...
            <div className="space-y-2">
              <label className="flex items-center text-xs cursor-pointer">
                <input
                  ...
                  checked={
                    editingBlock.targetType === "predicates" ||
                    !editingBlock.targetType
                  }
                  onChange={() =>
                    setEditingBlock((prev) => ({
                      ...prev,
                      targetType: "predicates",
                    }))
                  }
              ...
              <select
                ...
                onChange={(e) =>
                  setEditingBlock((prev) => ({
                    ...prev,
                    quantifier: e.target
                      .value as NeighborhoodBlock["quantifier"],
                  }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditingBlock((prev) => ({
                    ...prev,
                    relation: e.target.value as NeighborhoodBlock["relation"],
                  }))
                }
                className="w-full text-xs px-2 py-1 border border-gray-300 rounded focus:ring-violet-500"
              >
    ...
    <div className="relative border-2 border-violet-200 rounded-lg p-3 bg-gradient-to-r from-violet-50 to-violet-100/30">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <div
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-md border text-xs font-medium ${getQuantifierColor(block.quantifier)}`}
          >
          ...
          {block.targetType === "all_predicates" ? (
            <div className="inline-flex items-center gap-1 px-3 py-1.5 rounded border text-xs bg-gradient-to-r from-green-100 to-emerald-100 border-green-200">
              ...
              <span className="font-semibold text-green-800">
                All {filterItems.length} predicates
              </span>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-1">
              ...
              >
                {block.resultMode === "primary_only"
                  ? "Primary only"
                  : `Primary + ${block.projectionVariable || "neighbors"}`}
              </span>
            </div>
          </div>
