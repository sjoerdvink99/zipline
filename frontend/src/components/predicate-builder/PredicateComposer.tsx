...
import { NeighborhoodConstraintBlock, type NeighborhoodBlock } from './constraints/NeighborhoodConstraintBlock';
import { ProjectionResults, type StructuredProjectionResult } from '../results/ProjectionResults';
...
interface PredicateComposerProps {
  ...
  onFilterApply: (matchingNodeIds: string[], projectionResults?: any[]) => void;
  onFilterClear: () => void;
  sessionId?: string;
}
...
export function PredicateComposer({
  ...
  const {
    queryCanvas: { neighbourhoodConstraints },
    projection: { enabled: projectionEnabled, results: projectionResults },
    currentEvaluation,
  ...
  const createNeighborhoodBlock = useCallback(() => {
    ...
    const newBlock: NeighborhoodBlock = {
      ...
      targetType: selectedPredicateIds.length === filterItems.length ? 'all_predicates' : 'predicates',
      quantifier: 'ALL',
      relation: 'neighbors',
      constraint: {
        type: 'attribute',
  ...
  if (typeof window !== 'undefined') {
    (window as any).predicateSelectionMode = {
  ...
  const hasResults = currentEvaluation?.matching_nodes.length > 0;
  const hasNoMatches = currentEvaluation && currentEvaluation.matching_nodes.length === 0 && validationErrors.length === 0;
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <h4 className="text-sm font-medium text-red-800 mb-1">
                Expression Validation ({validationErrors.length} issue{validationErrors.length !== 1 ? 's' : ''})
              </h4>
      ...
      <div className="space-y-3">
        {!isSelectionMode ? (
          <button
            ...
            title={hasPredicates ? "Add neighborhood constraint to selected predicates" : "Add predicates first"}
          >
          ...
          <div className="flex items-center gap-3 p-3 bg-violet-50 border border-violet-200 rounded-lg">
            <div className="flex-1">
              <span className="text-sm font-medium text-violet-800">
                Select predicates for constraint ({selectedPredicateIds.length} selected)
              </span>
              <p className="text-xs text-violet-600 mt-1">
                Click on predicate pills above to select them for the neighborhood constraint.
              </p>
            </div>
        ...
        <div className="space-y-3">
          ...
          {neighbourhoodConstraints.map((constraint) => (
            <NeighborhoodConstraintBlock
              ...
              block={constraint}
              filterItems={filterItems}
              onUpdate={(updatedBlock) => updateNeighborhoodConstraint(constraint.id, updatedBlock)}
      ...
      {hasResults && (
        <div className="space-y-3">
          ...
          <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700">Result Mode:</span>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700 mb-2">
                Your predicate expression was evaluated successfully, but no nodes in the current dataset satisfy the specified criteria.
              </p>
              <div className="text-xs text-amber-600 bg-amber-100 border border-amber-200 rounded px-2 py-1 inline-block">
                <span className="font-medium">Expression:</span> {currentEvaluation?.expression}
              </div>
            </div>
          </div>
        </div>
      ...
      {!hasPredicates && !hasConstraints && (
        <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
          <div className="text-center">
            <svg className="w-12 h-12 text-blue-500 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
            ...
            <p className="text-sm text-blue-700 mb-4">
              Drag predicates from the topology and attribute panels to start building your cross-space query.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              <div className="p-3 bg-white border border-blue-200 rounded">
                <div className="font-medium text-blue-800 mb-1">1. Add Predicates</div>
                <div className="text-blue-600">Drag topology and attribute filters</div>
              </div>
              <div className="p-3 bg-white border border-blue-200 rounded">
                <div className="font-medium text-blue-800 mb-1">2. Add Constraints</div>
                <div className="text-blue-600">Create neighborhood relationships</div>
              </div>
              <div className="p-3 bg-white border border-blue-200 rounded">
                <div className="font-medium text-blue-800 mb-1">3. Evaluate</div>
                <div className="text-blue-600">Generate cross-space results</div>
              </div>
            </div>
          </div>
        </div>
