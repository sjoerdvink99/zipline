...
export function FilterBuilder({
  ...
  const [editingConstraint, setEditingConstraint] = useState<string | null>(
    null,
  );
        ...
        if (data) {
          ...
          if (dragData.type === "filter-item" && dragData.filterItem) {
            ...
            filterItem = {
              ...
              type: dragData.legacyType,
              predicate: dragData.predicate,
              description:
                dragData.predicate.description ||
                `${dragData.predicate.attribute} filter`,
  ...
  useEffect(() => {
    ...
    return () =>
      (window as EventTarget).removeEventListener(
        "gb:add-filter-item",
        handleAddFilterItem as EventListener,
      );
  }, [addPredicate]);
  ...
  const hasResults =
    evaluation?.matching_nodes && evaluation.matching_nodes.length > 0;
  ...
  const handleContextMenu = useCallback(
    (item: FilterItem, position: { x: number; y: number }) => {
          ...
          {!hasPredicates && !hasConstraints ? (
            <div className="text-center py-2 w-full">
              ...
              <p className="text-[9px] text-gray-400">
                Drag predicates from topology and attribute panels to create
                  ...
                  <button
                    onClick={() => {
                      const constraintAfterPredicate = {
                        ...
                        targetType: "predicates" as const,
                    ...
                    <button
                      onClick={() => {
                        const allPredicatesConstraint = {
                          ...
                          targetType: "all_predicates" as const,
                          quantifier: "ALL" as const,
                          relation: "neighbors" as const,
                          constraints: [
                            {
                ...
                <div key={constraint.id} className="flex items-center gap-2">
                  ...
                  {editingConstraint === constraint.id ? (
                    ...
                    <>
                          ...
                          <div className="text-xs text-violet-600">
                            {(() => {
                              if (
                                ...
                                (
                                  constraint as NeighborhoodConstraint & {
                                    constraint?: {
                                      attribute: string;
                                      operator: string;
                                      value?: string;
                      ...
                      <button
                        onClick={() => {
                          const nextConstraint: NeighborhoodConstraint = {
                            id: `constraint_${Date.now()}`,
                            targetPredicateIds: [constraint.id],
                            targetType: "constraints",
                            parentConstraintId: constraint.id,
                            quantifier: "SOME",
                            relation: "neighbors",
                            constraints: [
                              {
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Save this predicate combination as a reusable filter.
              </p>
              <div className="mb-4">
                <label
                  htmlFor="filter-name"
