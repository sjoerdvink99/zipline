...
import type { ConstraintItem, SetOperation } from "../../../types/fol";
...
interface NestedConstraintPillProps {
  ...
  isDraggable?: boolean;
  isDropZone?: boolean;
  onPillDrop?: (draggedItem: ConstraintItem, targetIndex: number) => void;
}
...
const formatConstraintText = (constraint: ConstraintItem): string => {
  ...
  if (
    ...
    "value2" in constraint.value
  ) {
    const val = constraint.value as { value2: string | number };
    text += ` ${constraint.value} and ${val.value2}`;
  } else {
...
export function NestedConstraintPill({
  ...
  const style =
    typeStyles[constraint.type as keyof typeof typeStyles] ||
    typeStyles.attribute;
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    if (
      data.startsWith("predicate-") ||
      data.startsWith("nested-constraint-")
    ) {
    ...
    <div className="flex items-center shrink-0">
        ...
        <>
          {operation === "not" ? (
            <button
              ...
              className="mx-1 px-1.5 py-0.5 text-xs font-mono font-bold text-red-500 hover:text-red-700 hover:bg-red-50 rounded transition-colors border border-red-200"
      ...
      >
        <div className="flex items-center gap-1">
          <span
            className={`w-1.5 h-1.5 rounded-full shrink-0 ${style.dot} ring-1 ring-white shadow-sm`}
          />
          {isDraggable && (
            <svg
              className="w-2 h-2 text-gray-300 hover:text-gray-500 cursor-grab active:cursor-grabbing"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
